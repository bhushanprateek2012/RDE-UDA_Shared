...
for i = 1:size(rel_class,1)
    ...
    % Skip triplet if any class has no matching samples (e.g. missing from pseudo-labels)
    if l1 == 0 || l2 == 0 || l3 == 0
        cnt = cnt + n;
        continue;
    end
...
% Drop placeholder rows left by skipped triplets (missing classes)
relative = relative(any(relative,2), :);
equality = equality(any(equality,2), :);

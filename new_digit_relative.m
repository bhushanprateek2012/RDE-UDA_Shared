...
fileID = fopen('digit_data.csv', 'w');
dataset = {'USPS_vs_MNIST', 'MNIST_vs_USPS'};

% Parameter initialization
params = struct('delta', 0.8, 'NN', 5, 'k', 70, 'eta', 10^(-1), 'sigma', 7, ...
                'ck', 128, 'no_reps', 5);
...
for idx = 2
       ...
       A0 = A0+1*computeRelativeLoss(X_dst, relative, params.ck);
        ...
        fprintf(fileID, '%s %f\n', strcat(dataset{idx}, '.csv'), max(AC));
path = ['NewResults/', dataset{idx}, '.mat'];
accuracy=max(AC)

        save(path, 'X_src', 'X_dst', 'Z_src','Z_dst','Y_src', 'Y_dst', 'accuracy');
end
